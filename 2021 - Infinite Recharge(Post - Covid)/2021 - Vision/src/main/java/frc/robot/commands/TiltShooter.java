...
public class TiltShooter extends CommandBase {
  ...
  private Camera2 cam;
  public TiltShooter(FlyWheel input1, Camera2 input2) {
  ...
  @Override
  public void execute() {
    getDisplacement = flywheel.getAngle();
    if(Math.abs(getDisplacement) > Math.abs(targetAngle)) {
      done = true;
    }
    if(targetAngle > 0){
      flywheel.setTiltSpeed(-1);
    }
    else{
      flywheel.setTiltSpeed(1);
    }
